...
define('APP_SITE_NAME', "CKD");
define('APP_META_COPYRIGHT', null);

define('CONFIG_DEBUG', true);
define('CONFIG_COMMENTS', true);
...
function debug( $vars, $exit = false ) {
	if( !empty($vars) & CONFIG_debug==true) {
		echo '<pre>';
		print_r( $vars );
		echo '</pre>';
		if( $exit ) {
...
// -----------------------------------------------------------------------
function comment( $vars, $exit = false ) {
	if( !empty($comment) & CONFIG_COMMENTS==true) {
		echo '<!-- ';
...
class Contact {
	...
	private static $SETTINGS = Array(
		'email_to' => Array('careers@ckdtech.co'),
		'email_subject' => "CKD Tech Contact Form Submission",
